! $MYHPSC/homeworks/hw5/test3.f90

program test3

    use omp_lib

    use quadrature3, only: trapezoid, simpson, error_table
    use functions, only: f, fevals, k

    implicit none
    real(kind=8) :: a,b,int_true,total_work,perc_work
    integer :: nvals(12)
    integer :: i, nthreads
...
100 format("Using ",i2," threads")
 ...
 12 format("CPU time = ",f12.8, " seconds")
    ...
    total_work = sum(fevals)
    do i=0,nthreads-1
        perc_work = (fevals(i)/total_work)*100
        print 101,  i, fevals(i), perc_work
101     format("fevals by thread ",i2,": ",i13," ",f12.8,"%")
        enddo
